...
SidebandNativeHandle::SidebandNativeHandle(bool inOwnsFd)
{
    ...
    ALOGE("%s:%i, create SidebandNativeHandle %p at client side,  inOwnsFd=%d",
    ...
    ALOGE("%s:%i finish, this=%p, ID=(%d:%d), planeId:%d", __FUNCTION__,
          __LINE__, this,  clientTid, index, planeId);
}

SidebandNativeHandle::SidebandNativeHandle(const native_handle *handle, bool inOwnsFd)
{
    ALOGE("%s:%i, create SidebandNativeHandle %p at server side, handle=%p, inOwnsFd=%d",
